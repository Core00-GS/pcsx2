...
void GSRendererHW::EmulateDATESelectMethod(DATEOptions& date_options, GSTextureCache::Target* rt, int& blend_alpha_min, int& blend_alpha_max)
{
	...
	else if ((features.texture_barrier || features.multidraw_fb_copy) && IsCoverageAlphaSupported())
	{
	...
	else if ((features.texture_barrier || features.multidraw_fb_copy) && m_texture_shuffle)
	{
...
void GSRendererHW::EmulateDATEGetConfig(DATEOptions& date_options, bool scale_rt_alpha, GSDevice::RecycledTexture& temp_ds)
{
	...
	{
		const bool need_barrier = m_conf.require_one_barrier || (m_conf.require_full_barrier && (features.texture_barrier || features.multidraw_fb_copy));
...
void GSRendererHW::DetermineBarriers(GSTextureCache::Target* rt)
{
	...
	if (features.framebuffer_fetch)
	{
		...
		const bool need_barriers_for_depth = m_conf.ps.IsFeedbackLoopDepth() &&
